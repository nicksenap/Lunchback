...
# Pick out lists that length ls more then 2
...
"""
	...
	left_over_list = left_over(target_list, x_list)

	pr_res = anti_duplicate(x_to_two(x_list))
	...
	while flag:
		...
		if choose.upper() == 'A':
			...
			print('Left over:')
			print('====================')
			A = left_over_list[:len(left_over_list)/2]
			B = left_over_list[len(left_over_list)/2:]
